...
import { Art } from '../Art'
import { ALL_ART_KEYS, ART_ALT } from '../registry'
...
describe('캐릭터 티어별 실루엣 (Ruling 57)', () => {
  ...
  const shapesByTier = tiers.map(t => {
    const { container } = render(<>{(() => {
      const C = makeCharacter(t, 'normal', '테스트')
      return <C />
    })()}</>)
  ...
  it('mood(normal/shaken/joy) 분기는 그대로 유지된다', () => {
    const Normal = makeCharacter(0, 'normal', '테스트')
    const Shaken = makeCharacter(0, 'shaken', '테스트')
    const Joy = makeCharacter(0, 'joy', '테스트')
...
// 최종 리뷰 Minor 4 — 캐릭터 18종의 aria-label이 전부 "캐릭터"로 같아, 스크린리더로는
// 티어도 표정도 구분되지 않았다. ART_ALT의 한국어 설명이 실제 svg에도 붙는지 고정한다.
describe('캐릭터 svg의 접근성 이름 (최종 리뷰 Minor 4)', () => {
  it('18개 키의 aria-label이 ART_ALT와 같고 서로 다르다', () => {
    const labels = new Set<string>()
    for (const key of ALL_ART_KEYS.filter(k => k.startsWith('char.'))) {
      const { container } = render(<Art id={key} />)
      const label = container.querySelector('svg')?.getAttribute('aria-label')
      expect(label, key).toBe(ART_ALT[key])
      expect(label, `${key}의 접근성 이름이 비었다`).toBeTruthy()
      labels.add(label!)
    }
    expect(labels.size).toBe(18)
    expect(labels.has('캐릭터')).toBe(false)
  })
})
