...
describe('완전 불투명 장면 3종 — 런타임 인라인 스타일 불변식 (Ruling 28, Fix Round 1 Major 1)', () => {
  for (const scene of OPAQUE_SCENES) {
    describe(scene.name, () => {
      it(`${scene.name} 장면 배경(${scene.testId})은 인라인 opacity를 전혀 갖지 않는다`, () => {
        scene.render()
        const el = screen.getByTestId(scene.testId)
        expect(el.style.opacity, `${scene.testId} 요소에 인라인 opacity가 걸려 있다: "${el.style.opacity}"`).toBe('')
      })

      it(`${scene.name} 장면 배경(${scene.testId})은 인라인 animation을 전혀 갖지 않는다`, () => {
        scene.render()
        const el = screen.getByTestId(scene.testId)
        expect(el.style.animation, `${scene.testId} 요소에 인라인 animation이 걸려 있다: "${el.style.animation}"`).toBe('')
      })
    })
  }
})
